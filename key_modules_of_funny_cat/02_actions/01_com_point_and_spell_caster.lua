...
local FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER = Action({priority = 10})   --- 距离 和 目标物体的 碰撞体积有关，为 0 也没法靠近。
FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER.id = "FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER"
FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER.strfn = function(act) --- 客户端检查是否通过,同时返回显示字段
    local item = act.invobject
    local target = act.target
    local doer = act.doer
    local pos = act.pos or {}
    if item then
        local replica_com = item.replica.funny_cat_com_point_and_target_spell_caster or item.replica._.funny_cat_com_point_and_target_spell_caster
        if replica_com then
...
FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER.fn = function(act)    --- 只在服务端执行~
    local item = act.invobject
    local target = act.target
    local doer = act.doer
    local pos = act.pos or {}

    if item and doer and item.components.funny_cat_com_point_and_target_spell_caster then
        local replica_com = item.replica.funny_cat_com_point_and_target_spell_caster or item.replica._.funny_cat_com_point_and_target_spell_caster
        if replica_com and replica_com:Test(doer, target, pos.local_pt,true) then
            return item.components.funny_cat_com_point_and_target_spell_caster:CastSpell(doer, target,pos.local_pt)
        end
    end
    return false
end
AddAction(FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER)
...
---- 为了避免多个inst用同一个动作造成冲突，调用前清除一些额外插入的参数。（没法用deepcopy 只能这样）
    ...
    local function GetAction()
        local temp_action = ACTIONS.FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER
...
AddComponentAction("POINT", "funny_cat_com_point_and_target_spell_caster",function(item, doer, pos, actions, right_click)   ------ 指定坐标位置用。
...
AddComponentAction("EQUIPPED", "funny_cat_com_point_and_target_spell_caster",function(item, doer, target, actions, right_click)    --- 装备后多个技能
...
local handler_fn = function(player)
    ...
    if creash_flag == true then
        ...
        print("error in FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER ActionHandler")
        print(ret)
    end
    return "give"
end

AddStategraphActionHandler("wilson",ActionHandler(FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER,function(player)
    return handler_fn(player)
end))
AddStategraphActionHandler("wilson_client",ActionHandler(FUNNY_CAT_COM_POINT_AND_TARGET_SPELL_CASTER, function(player)
    return handler_fn(player)
end))
