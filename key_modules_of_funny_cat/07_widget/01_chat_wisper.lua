...
--[[
    ...
    ChatHistory:AddToHistory({flag = "funny_cat" , ChatType = ChatTypes.Message , m_colour = {0,0,255} , s_colour = {255,255,0}},nil,nil,"NPC","656565",{0,255,0})
...
ChatHistory.GenerateChatMessage_old_funny_cat = ChatHistory.GenerateChatMessage
ChatHistory.GenerateChatMessage = function(self,Chat_type, sender_userid, sender_netid, sender_name, message, colour, icondata, whisper, localonly, text_filter_context)
    local funny_cat_cmd_table = {}
    if Chat_type and type(Chat_type) == "table" and Chat_type.flag == "funny_cat" then
        funny_cat_cmd_table = deepcopy(Chat_type)
        Chat_type = funny_cat_cmd_table.ChatType or ChatTypes.Message                        ---- 文本类型
        localonly = true                                                                ---- 只在本地
        icondata = funny_cat_cmd_table.icondata or icondata -- or "default"                     ---- 图标
        sender_name = funny_cat_cmd_table.sender_name or sender_name                         ---- 发送者名字
        message = funny_cat_cmd_table.message or message                                     ---- 文字内容
        colour = funny_cat_cmd_table.s_colour or funny_cat_cmd_table.m_colour or {255/255,255/255,255/255}    ---- 颜色初始化。
    end

    local ret_chat_message = self:GenerateChatMessage_old_funny_cat(Chat_type, sender_userid, sender_netid, sender_name, message, colour, icondata, whisper, localonly, text_filter_context)
    
    if funny_cat_cmd_table.flag == "funny_cat" then
        if funny_cat_cmd_table.m_colour then            
            ret_chat_message.m_colour = funny_cat_cmd_table.m_colour --- 文本颜色
        end
        if funny_cat_cmd_table.s_colour then
            ret_chat_message.s_colour = funny_cat_cmd_table.s_colour --- 文本颜色
        end
    end
...
AddPlayerPostInit(function(inst)
    ...
    inst:ListenForEvent("funny_cat_event.whisper",function(_,_table)
        if _table and _table.message then
            _table.flag = "funny_cat"
