-- TUNING["Forward_In_Predicament.Config"] = {}
-- TUNING["Forward_In_Predicament.Config"].Language = GetModConfigData("LANGUAGE")


TUNING.FUNNY_CAT_CONFIG = {}

TUNING.FUNNY_CAT_CONFIG.MAP_SIZE_BIG = GetModConfigData("MAP_SIZE_BIG") -- 地图大尺寸
------------------------------------------------------------------------------------------------------------------------------------------------------
TUNING.FUNNY_CAT_CONFIG.MAP_SIZE_BIG = GetModConfigData("MAP_SIZE_BIG") -- 地图大尺寸
------------------------------------------------------------------------------------------------------------------------------------------------------
--- 鬼队
    TUNING.FUNNY_CAT_CONFIG.CLEANING_BROOM_RANGE = GetModConfigData("CLEANING_BROOM_RANGE") -- 扫把扫描半径
    TUNING.FUNNY_CAT_CONFIG.CLEANING_BROOM_RUN_SPEED_MULTIPLIER = GetModConfigData("CLEANING_BROOM_RUN_SPEED_MULTIPLIER") -- 扫把跑路倍数
------------------------------------------------------------------------------------------------------------------------------------------------------
TUNING.FUNNY_CAT_CONFIG.ALLOW_SERVER_MODS = GetModConfigData("ALLOW_SERVER_MODS") -- 允许服务器MOD

TUNING.FUNNY_CAT_CONFIG.CARD_HOT_KEY_1 = GetModConfigData("CARD_HOT_KEY_1")  -- 卡槽快捷键1
TUNING.FUNNY_CAT_CONFIG.CARD_HOT_KEY_2 = GetModConfigData("CARD_HOT_KEY_2")  -- 卡槽快捷键2
TUNING.FUNNY_CAT_CONFIG.CARD_HOT_KEY_3 = GetModConfigData("CARD_HOT_KEY_3")  -- 卡槽快捷键3

TUNING.FUNNY_CAT_CONFIG.EMOJI_HOT_KEY = GetModConfigData("EMOJI_HOT_KEY")  -- 表情包快捷键